...
public class Controller {
	...
	public static void listarInterfaces() {
		if (ReconocerSO.isWindows()) {
			...
			try {
				...
				BufferedReader lector = new BufferedReader(
						new InputStreamReader(proceso.getInputStream(), Charset.forName("CP850")));
				String linea;
		...
		/* UNIX */
			...
			try {
				...
				while ((linea = lector.readLine()) != null) {
					if (!linea.startsWith(" ") && !linea.isBlank()){
						interfaces = linea.split(":");
						interfaces[1] = interfaces[1].substring(1);
						buffer.append(interfaces[1] + "\n");
					}
				}
	...
	public static void IPInterfaz(String interfaz) {
		...
		/* UNIX */
			...
			try {
				String comando = "ip a | grep " + interfaz;
				pB.command("bash", "-c", comando);
				Process proceso = pB.start();
				StringBuilder buffer = new StringBuilder();
				BufferedReader lector = new BufferedReader(
		...
		/* UNIX */
			...
			try {
				...
				while ((linea = lector.readLine()) != null) {
					...
					if (entrar && linea.contains("link/ether")) {
						linea = linea.substring(4);
						IPv4 = linea.split(" ");
						buffer.append(IPv4[1] + "\n");
						entrar = false;
	...
	public static boolean conexionInternet() {
		...
		/* UNIX */
			...
			String comando = "ping 8.8.8.8 -c 1";
			pB.command("bash", "-c", comando);
			try {
				Process proceso = pB.start();
				BufferedReader lector = new BufferedReader(
